...
export default function ProductPage ( props )
{
	...
	const [ freeGames, setFreeGames ] = useState( localStorage.getItem( "freeGame" ) );
 ...
 const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
   setDateDiff( diffDays );
	...
	// if (payment.props.children !== undefined){ 
	//var sgst = ( ( payment.props.children * 9 ) / 100 )
	//var cgst = ( ( payment.props.children * 9 ) / 100 )
	// total = payment.props.children + sgst + cgst
// }
 ...
 src={`https://backendrentkar.herokuapp.com${item.image}`} alt={`image${item}`}
 alt="First slide"
 />
 </Carousel.Item>
 )
 ...
 */}
 ...
 <div className="select__games">
          ...
          <div className="select__free__games">
            ...
            >
              <img src={ JSON.parse(localStorage.getItem("freeGame1")) !== null ? JSON.parse(localStorage.getItem("freeGame1")).img : add} />
            </div>
            <div
              className="btn btn-outline-dark games"
              onClick={() => setGameModalShow2(true)}
            >
              <img src={ JSON.parse(localStorage.getItem("freeGame2")) !== null ? JSON.parse(localStorage.getItem("freeGame2")).img : add} />
              {/* <img src={add} /> */}
            </div>
            <AddGamesModal
              show={gameModalShow1}
